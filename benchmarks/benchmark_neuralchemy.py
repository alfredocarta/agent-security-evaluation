...
DATASET_PATH = "benchmarks/neuralchemy_prompt_injection.json"
RESULTS_PATH = "benchmarks/neuralchemy_results_full.json"
...
print(f"Source: neuralchemy/Prompt-injection-dataset (29 attack categories, 2025 techniques)")
...
def run_config(samples, classify_fn, label=""):
    ...
    by_severity = defaultdict(empty_counts)
    start = datetime.now()
    ...
    for s in tqdm(samples, desc=label, unit="sample", dynamic_ncols=True, file=sys.stdout):
        ...
        update_counts(by_category[s["category"]], inj, blocked, lat)
        update_counts(by_severity[s["severity"]], inj, blocked, lat)
    end = datetime.now()
    print(f"[{end.strftime('%H:%M:%S')}] Done: {label} in {(end-start).total_seconds():.0f}s")
    metrics = finalize_counts(counts)
    metrics["by_category"] = {k: finalize_counts(v) for k, v in by_category.items()}
    metrics["by_severity"] = {k: finalize_counts(v) for k, v in by_severity.items()}
    return metrics
...
print()
print("=== Benchmark: neuralchemy/Prompt-injection-dataset ===")
print(f"Dataset: {len(samples)} samples ({injection_count} injection, {benign_count} benign)")
print()
...
print()
print("=== Results by Category (ASF L1.5) ===")
l15 = benchmark_rows[0][1]
print(f"{'Category':<35} {'Recall':>8} {'FPR':>6} {'F1':>6} {'N':>5}")
print("-" * 60)
for cat in sorted(l15["by_category"], key=lambda c: -l15["by_category"][c]["n_samples"]):
    m = l15["by_category"][cat]
    print(f"{cat:<35} {m['recall']:>7.1%} {m['fpr']:>5.1%} {m['f1']:>6.3f} {m['n_samples']:>5}")

print()
print("=== Results by Severity (ASF L1.5) ===")
print(f"{'Severity':<15} {'Recall':>8} {'FPR':>6} {'F1':>6} {'N':>5}")
print("-" * 40)
for sev in sorted(l15["by_severity"]):
    m = l15["by_severity"][sev]
    print(f"{sev:<15} {m['recall']:>7.1%} {m['fpr']:>5.1%} {m['f1']:>6.3f} {m['n_samples']:>5}")
...
results = {
    ...
    "by_category_l15": {k: {"recall": round(v["recall"], 4), "fpr": round(v["fpr"], 4),
                             "f1": round(v["f1"], 4), "n_samples": v["n_samples"]}
                        for k, v in l15["by_category"].items()},
    "by_severity_l15": {str(k): {"recall": round(v["recall"], 4), "fpr": round(v["fpr"], 4),
                                  "f1": round(v["f1"], 4), "n_samples": v["n_samples"]}
                        for k, v in l15["by_severity"].items()},
}
