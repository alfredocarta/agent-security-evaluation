...
DATASET_PATH = "benchmarks/spml_prompt_injection.json"
RESULTS_PATH = "benchmarks/spml_results_full.json"
...
print(f"Source: reshabhs/SPML_Chatbot_Prompt_Injection (User Prompt field)")
print(f"Degree distribution: { {k: degrees[k] for k in sorted(degrees)} }")
...
def run_config(samples, classify_fn, label=""):
    counts = empty_counts()
    by_degree = defaultdict(empty_counts)
    start = datetime.now()
    ...
    for s in tqdm(samples, desc=label, unit="sample", dynamic_ncols=True, file=sys.stdout):
        ...
        update_counts(by_degree[s["degree"]], inj, blocked, lat)
    end = datetime.now()
    print(f"[{end.strftime('%H:%M:%S')}] Done: {label} in {(end-start).total_seconds():.0f}s")
    metrics = finalize_counts(counts)
    metrics["by_degree"] = {d: finalize_counts(c) for d, c in by_degree.items()}
    return metrics
...
print()
print("=== Benchmark: SPML Chatbot Prompt Injection ===")
print(f"Dataset: {len(samples)} samples ({injection_count} injection, {benign_count} benign)")
print()
...
print()
print("=== Results by Injection Degree (ASF L1.5) ===")
l15_metrics = benchmark_rows[0][1]
print(f"{'Degree':<10} {'Recall':>8} {'FPR':>6} {'F1':>6} {'N':>5}")
print("-" * 35)
for d in sorted(l15_metrics["by_degree"].keys()):
    m = l15_metrics["by_degree"][d]
    print(f"{d:<10} {m['recall']:>7.1%} {m['fpr']:>5.1%} {m['f1']:>6.3f} {m['n_samples']:>5}")
