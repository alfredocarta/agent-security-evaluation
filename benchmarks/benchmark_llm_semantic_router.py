...
DATASET_PATH = "benchmarks/llm_semantic_router_jailbreak.json"
RESULTS_PATH = "benchmarks/llm_semantic_router_results_full.json"
...
label_texts = Counter(s["label_text"] for s in samples)

print(f"Dataset: {len(samples)} samples ({injection_count} injection, {benign_count} benign)")
print(f"Source: llm-semantic-router/jailbreak-detection-dataset")
print(f"Label text distribution: {dict(label_texts.most_common(10))}")
...
def run_config(samples, classify_fn, label=""):
    counts = empty_counts()
    by_label_text = defaultdict(empty_counts)
    start = datetime.now()
    ...
    for s in tqdm(samples, desc=label, unit="sample", dynamic_ncols=True, file=sys.stdout):
        ...
        update_counts(by_label_text[s["label_text"]], inj, blocked, lat)
    end = datetime.now()
    print(f"[{end.strftime('%H:%M:%S')}] Done: {label} in {(end-start).total_seconds():.0f}s")
    metrics = finalize_counts(counts)
    metrics["by_label_text"] = {k: finalize_counts(v) for k, v in by_label_text.items()}
    return metrics
...
print()
print("=== Benchmark: llm-semantic-router/jailbreak-detection-dataset ===")
print(f"Dataset: {len(samples)} samples ({injection_count} injection, {benign_count} benign)")
print()
...
print()
print("=== Results by Label Text (ASF L1.5) ===")
l15 = benchmark_rows[0][1]
print(f"{'Label':<25} {'Recall':>8} {'FPR':>6} {'F1':>6} {'N':>5}")
print("-" * 50)
for lt in sorted(l15["by_label_text"], key=lambda x: -l15["by_label_text"][x]["n_samples"]):
    m = l15["by_label_text"][lt]
    print(f"{lt:<25} {m['recall']:>7.1%} {m['fpr']:>5.1%} {m['f1']:>6.3f} {m['n_samples']:>5}")

results = {
    "dataset": {"path": DATASET_PATH, "n_samples": len(samples),
                "n_injection": injection_count, "n_benign": benign_count,
                "label_text_distribution": dict(label_texts)},
    ...
    "by_label_text_l15": {k: {"recall": round(v["recall"], 4), "fpr": round(v["fpr"], 4),
                               "f1": round(v["f1"], 4), "n_samples": v["n_samples"]}
                          for k, v in l15["by_label_text"].items()},
}
