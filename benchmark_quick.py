...
from interceptor import hardened_interceptor, _semantic_probe
from hardening import _classifier_gate_score

# Suppress logs
os.environ['ASF_DISABLE_FASTPATH'] = 'false'
...
def run_benchmark(data, name, sample_size=None):
    if sample_size:
    ...
    for i, item in enumerate(data):
        ...
        # Run detection
        probe_hit = _semantic_probe(text)
        l15_score = _classifier_gate_score(text)
        
        # Simple decision: block if probe hit OR l15_score > 0.3
        predicted_dangerous = probe_hit or l15_score > 0.35
...
print("\\n" + "="*70)
print("BENCHMARK: deepset + OPI subset (L1.5 + semantic probe + ONNX parallel)")
print("="*70)
...
opi_scaled = {
    'total': 67200,
    'tp': int(opi_results['tp'] * 33.6),
    'fp': int(opi_results['fp'] * 33.6),
    'tn': int(opi_results['tn'] * 33.6),
    'fn': int(opi_results['fn'] * 33.6),
    'recall': opi_results['recall'],
    'fpr': opi_results['fpr'],
    'precision': opi_results['precision'],
    'f1': opi_results['f1'],
    'elapsed_s': opi_results['elapsed_s'] * 33.6,
